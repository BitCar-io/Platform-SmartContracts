...
contract("Escrow Tests", async (accounts) => {
    ...
    it("should claim platform tokens", async function () {
        ...
        var result = await platformHelper.Escrow.claimPlatformTokens({ from: trader0 });

        var userPlatformBalanceAfter = await platformHelper.BitcarToken.balanceOf(trader0);

        assert.isTrue(result.tx.length === 66);

        assert.isTrue(userPlatformBalanceAfter - userPlatformBalanceBefore === 86);
    });

    it("should NOT claim platform tokens twice", async function () {
        await truffleAssert.reverts(platformHelper.Escrow.claimPlatformTokens(), "User already claimed all asset tokens");
    });
